...
int heartbeat_step(int sockfd, struct sockaddr_in *clntaddr,
                   socklen_t *clnt_leng, int HB_INTERVAL,
...
int main(int argc, char *argv[]) {
  ...
  Error_Type =
      atoi(argv[2]); // 고장발생 1-> 고장 없음(전원 절체), 2-> 미션 속도
                     // 느림(WDT), 3-> 통신 두절 ,4-> 온도 고장, 5-> 급격한 온도
                     // 변화 고장, 6-> 메모리 고장, 7-> 유후 메모리 부족  ,8->
                     // 전압 고장, 9-> 급격한 전압 변화 고장
  // -p 플래그 및 그 뒤의 미션 프로그램 이름 처리
  for (int i = 3; i < argc;
       i++) { // argv[1]은 이미 HB_INTERVAL로, argv[1]은 이미 Error_Type로
              // 사용되었으므로, i는 3부터 시작
...
void *Heartbeat(void *arg) {
  ...
  setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const void *)&bValid,
             sizeof(bValid)); // 소켓 초기 설정
  ...
  setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &read_timeout,
             sizeof read_timeout); // rcv 제한 시간
  // setsockopt(sockfd, SOL_SOCKET, SO_SNDTIMEO, &read_timeout, sizeof
  // read_timeout);
  ...
  while (1) {
    int should_break =
        heartbeat_step(sockfd, &clntaddr, &clnt_leng, HB_INTERVAL, stop, &ct,
...
int heartbeat_step(int sockfd, struct sockaddr_in *clntaddr,
  ...
  *clnt_leng = len;
  ...
  // 6) Transmit HB
  send_heartbeat();
  sendto(sockfd, snd_buff, (int)HBSIZE, 0, (struct sockaddr *)clntaddr,
         *clnt_leng);
